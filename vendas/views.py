...
def produto_list(request):
    ...
    context = {
        'produtos': produtos,
        'page_obj': produtos,
        'is_paginated': produtos.paginator.num_pages > 1,
    }
    return render(request, 'vendas/produto_list.html', context)
...
+    """
    ...
    # ✅ ENVIAR LISTA DE PRODUTOS PARA O <select> DO FORM
...
def venda_create(request):
    ...
    """
    if request.method == 'POST':                         # <-- ALTERAÇÃO
        ...
        if form.is_valid():
            ...
            is_ajax = (request.headers.get('X-Requested-With') == 'XMLHttpRequest'
                       or 'fetch' in str(request.META.get('HTTP_USER_AGENT', '')))
        ...
        form = VendaForm()                               # começa vazio no GET
